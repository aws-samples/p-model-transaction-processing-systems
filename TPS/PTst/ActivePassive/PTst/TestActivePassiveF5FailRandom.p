...
/*******************************************************************
This file contains test case for active-passive-fail-random scenario
********************************************************************/

test tcActivePassiveF5 [main=TestActivePassiveF5FailRandom]:
  assert ActivePassiveIsSafeAndLive in
  (union ActivePassive, { TestActivePassiveF5FailRandom });

//
// Test driver that checks the system with active-passive-fail-random scenario
//
...
// 16. outbound queue
machine TestActivePassiveF5FailRandom {
  ...
  start state Init {
    ...
    entry {
      ...
      systemConstants = getActivePassiveConstants();
			...
			receive {
				case eRecordSourceDNSFailoverQueueGenerateRecordsNotification: (input: (name: string, region: int, batch: int, count: int, success: bool)) {
			...
			while(i <= 16) {
					...
					if (i == 1) {
						...
						receive {
							case eQueueFailCompleted: (input: (name: string, region: int)) {
								print format ("Inbound queue fail completed");
							}
						}
					}
					else if (i == 2) {
						send system, eQueueAuroraQueueContainerFail, (name = systemConstants.inboundContainerName, region = 1);
					}
					else if (i == 3) {
						send system, eAuroraFail, (name = systemConstants.inboundDatabaseName, invoker = this);
						receive {
							case eAuroraFailCompleted: (input: (name: string)) {
          	...
          	send system, eQueueFail, (name = systemConstants.inboundPreprocessQueueName,  region = 1, invoker = this);
						...
						send system, eAuroraFail, (name = systemConstants.preprocessDatabaseName, invoker = this);
						receive {
							case eAuroraFailCompleted: (input: (name: string)) {
          	...
          	send system, eQueueFail, (name = systemConstants.preprocessCoreQueueName,  region = 1, invoker = this);
						receive {
							case eQueueFailCompleted: (input: (name: string, region: int)) {
								print format ("Preprocess-Core queue fail completed");
							}
						}
          }
					else if (i == 8) {
						send system, eQueueAuroraQueueContainerFail, (name = systemConstants.coreContainerName, region = 1);
					}
					else if (i == 9) {
						send system, eAuroraFail, (name = systemConstants.coreDatabaseName, invoker = this);
						receive {
							case eAuroraFailCompleted: (input: (name: string)) {
								print format ("Preprocess database fail completed");
							}
          	...
          	send system, eQueueFail, (name = systemConstants.corePostprocessQueueName,  region = 1, invoker = this);
						receive {
							case eQueueFailCompleted: (input: (name: string, region: int)) {
								print format ("Core-Postprocess queue fail completed");
							}
						}
          }
					else if (i == 11) {
						send system, eQueueAuroraQueueContainerFail, (name = systemConstants.postprocessContainerName, region = 1);
					}
					else if (i == 12) {
						send system, eAuroraFail, (name = systemConstants.postprocessDatabaseName, invoker = this);
						receive {
							case eAuroraFailCompleted: (input: (name: string)) {
          	...
          	send system, eQueueFail, (name = systemConstants.postprocessOutboundQueueName,  region = 1, invoker = this);
						receive {
							case eQueueFailCompleted: (input: (name: string, region: int)) {
								print format ("Postprocess-Outbound queue fail completed");
							}
						}
          }
					else if (i == 14) {
						send system, eQueueAuroraQueueContainerFail, (name = systemConstants.outboundContainerName, region = 1);
					}
					else if (i == 15) {
						send system, eAuroraFail, (name = systemConstants.outboundDatabaseName, invoker = this);
						receive {
							case eAuroraFailCompleted: (input: (name: string)) {
          	...
          	send system, eQueueFail, (name = systemConstants.outboundQueueName,  region = 1, invoker = this);
						receive {
							case eQueueFailCompleted: (input: (name: string, region: int)) {
								print format ("Outbound queue fail completed");
							}
						}
          }
				}
      ...
      publishingSuccessful = false;
			...
			while(i <= 16) {
				if (i in failIndexes) {
					...
					if (i == 1) {
						...
						send system, eAuroraRecover, (name = systemConstants.inboundDatabaseName, invoker = this);
            receive {
              case eAuroraRecoverCompleted: (input: (name: string, success: bool)) {
						...
						send system, eAuroraRecover, (name = systemConstants.preprocessDatabaseName,  invoker = this);
            receive {
              case eAuroraRecoverCompleted: (input: (name: string, success: bool)) {
              }
            }
						...
						send system, eAuroraRecover, (name = systemConstants.coreDatabaseName,  invoker = this);
            receive {
              case eAuroraRecoverCompleted: (input: (name: string, success: bool)) {
              }
            }
						...
						send system, eAuroraRecover, (name = systemConstants.postprocessDatabaseName,  invoker = this);
            receive {
              case eAuroraRecoverCompleted: (input: (name: string, success: bool)) {
              }
            }
						...
						send system, eAuroraRecover, (name = systemConstants.outboundDatabaseName,  invoker = this);
            receive {
              case eAuroraRecoverCompleted: (input: (name: string, success: bool)) {
              }
            }
			...
			send system, eQueueAuroraQueueContainerRecover, (name = systemConstants.coreContainerName, region = 1);
			send system, eQueueAuroraQueueContainerRecover, (name = systemConstants.postprocessContainerName, region = 1);
			send system, eQueueAuroraQueueContainerRecover, (name = systemConstants.outboundContainerName, region = 1);
