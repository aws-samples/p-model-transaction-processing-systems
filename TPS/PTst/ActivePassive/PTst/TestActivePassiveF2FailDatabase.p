...
/*********************************************************************
This file contains test case for active-passive-fail-database scenario
**********************************************************************/

test tcActivePassiveF2 [main=TestActivePassiveF2FailDatabase]:
  assert ActivePassiveIsSafeAndLive in
  (union ActivePassive, { TestActivePassiveF2FailDatabase });

//
// Test driver that checks the system with active-passive-fail-database scenario
//
...
// STEP 5: RECOVER PREPROCESS CONTAINER AS IT MIGHT HAVE GONE DOWN WHEN DATABASE RETURNED AN ERROR MESSAGE WHILE TRYING TO INSERT A RECORD
// STEP 6: PUBLISH 10 RECORDS AND CONFIRM THAT ALL 30 RECORDS WERE DELIVERED TO PREPROCESS CONTAINER
//

machine TestActivePassiveF2FailDatabase {
  ...
  start state Init {
    ...
    entry {
			...
			// sign up to receive notification from inbound preprocess queue when it receives 20 records (checking that inbound preprocess queue got 20 records as preprocess container will go down before processing 20 records, when database returns failure when trying to insert records)
